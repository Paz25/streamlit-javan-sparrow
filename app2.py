...
# Path GIF dekorasi yang tampil di atas kartu "Model Aktif".
# Letakkan file .gif di folder public/ di root project.
# Nama file dapat diganti; jika file tidak ditemukan, area ini tidak dirender.
DECO_GIF_PATH = Path("./public/cat-scuba.gif")
...
def _render_deco_gif(gif_path: Path) -> None:
    """
    Render GIF dekorasi sebagai elemen HTML <img> dengan rasio aspek 1:1.

    GIF dibaca dari disk, diencode ke base64, lalu diinjeksikan langsung ke
    dalam HTML agar Streamlit tidak mem-proxy atau mengubah formatnya.
    Wrapper CSS memastikan:
      - Lebar penuh kolom tempat fungsi ini dipanggil
      - Rasio aspek 1:1 (square), kelebihan dipotong dari tengah (object-fit: cover)
      - Sudut membulat selaras dengan .step-card di bawahnya
    """
    if not gif_path.exists():
        return  # file tidak ada — skip tanpa pesan error agar halaman tetap bersih

    try:
        gif_b64 = base64.b64encode(gif_path.read_bytes()).decode("utf-8")
    except OSError:
        return  # gagal baca — skip diam-diam
...
def run_interactive_pipeline(y_raw, variant_name, arch, feature, model_obj, label_meta, metadata):
    ...
    if arch == "rf":
        ...
        st.caption(
            "LSTM memproses sekuens temporal dari kiri ke kanan dan kanan ke kiri "
...
def main():
    ...
    with st.sidebar:
        ...
        arch_desc = {
            ...
            "LSTM + MelSpec": "2-layer LSTM · sekuens dari Mel-Spectrogram",
            "LSTM + MFCC":    "2-layer LSTM · sekuens dari matriks MFCC",
        }
    ...
    with col_info:
        ...
        # GIF dekorasi — selebar kolom, rasio 1:1, dari folder public/
        _render_deco_gif(DECO_GIF_PATH)
